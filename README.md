...
Серия дней (Streak) отслеживает непрерывные дни выполнения повторений:

- При запуске приложения на новый день проверяется перерыв
- Если прошла 1 день (вчера была активность) — streak сохраняется для восстановления сегодня
- Если прошло >1 дня — streak обнуляется сразу
- При первом повторении в новый день streak увеличивается (если не было перерыва) или начинается заново (если был перерыв)
- В течение дня повторения не влияют на streak, только на счётчик toDay

---

Recent Updates

- ✅ Исправлена логика обновления streak при пропуске дней
- ✅ Удалены все комментарии из кода (оставлены только @ts-nocheck)
- ✅ Оптимизирована синхронизация статистики между экранами
- ✅ Фиксирована проблема с несохранением статистики при повторении
